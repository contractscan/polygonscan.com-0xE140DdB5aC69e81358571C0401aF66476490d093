  ...
  PausableUpgradeable
{
  ...
  function calculateGasTokenAmount() public view returns (uint256) {
    ...
    // solhint-disable-next-line no-unused-vars
    (uint80 _e, int256 bnbAnswer, uint256 _f, uint256 _g, uint80 _h) = _maticAggregator.latestRoundData();
    uint256 convertedInGas = (outGasPrice * outGasLimit * uint256(maticAnswer)) / uint256(bnbAnswer);
